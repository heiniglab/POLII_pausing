...
parse.gene.annotations <- function(){
  ...
  #temp <- import("/storage_scratch/groups_epigenereg/datasets/RBPs/TEMP2/gencode.v19.chr_patch_hapl_scaff.annotation.gtf")
  #TxDb <- makeTxDbFromGFF(paste(INPUT, "GeneAnnotations/RefSeq/NCBI/", "GRCh37_latest_genomic.gff", sep =""))
...
###
load.RNAseq <- function(filter = F, min.reads = 0){
  ...
  filter.quantifications <- function(sub.quants){
    ...
    # Only retain transcripts with at least "min.reads" FPKM
    sub.quants <- sub.quants[sub.quants$FPKM > min.reads, ]
  ...
  total.quants.rep1 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[1], "/total/exp1/rep1/ENCFF424CXV.tsv", sep =""))
  total.quants.rep2 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[1], "/total/exp1/rep2/ENCFF073NHK.tsv", sep =""))
  cytosolic.quants.rep1 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[1], "/cytosolic/rep1/ENCFF720FHY.tsv", sep =""))
  cytosolic.quants.rep2 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[1], "/cytosolic/rep2/ENCFF569JWX.tsv", sep =""))
  insoluble_cytoplasmic.quants.rep1 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[1], "/insoluble_cytoplasmic/rep1/ENCFF895NOJ.tsv", sep =""))
  insoluble_cytoplasmic.quants.rep2 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[1], "/insoluble_cytoplasmic/rep2/ENCFF924BJS.tsv", sep =""))
  membrane.quants.rep1 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[1], "/membrane/rep1/ENCFF764QIL.tsv", sep =""))
  membrane.quants.rep2 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[1], "/membrane/rep2/ENCFF868MVT.tsv", sep =""))
  nuclear.quants.rep1 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[1], "/nuclear/rep1/ENCFF168FPZ.tsv", sep =""))
  nuclear.quants.rep2 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[1], "/nuclear/rep2/ENCFF619LZT.tsv", sep =""))
  cline.quants$K562 <- list(total.quants.rep1 = total.quants.rep1, total.quants.rep2 = total.quants.rep2,
                            cytosolic.quants.rep1 = cytosolic.quants.rep1, cytosolic.quants.rep2 = cytosolic.quants.rep2,
                            insoluble_cytoplasmic.quants.rep1 = insoluble_cytoplasmic.quants.rep1, insoluble_cytoplasmic.quants.rep2 = insoluble_cytoplasmic.quants.rep2,
                            membrane.quants.rep1 = membrane.quants.rep1, membrane.quants.rep2 = membrane.quants.rep2, 
                            nuclear.quants.rep1 = nuclear.quants.rep1, nuclear.quants.rep2 = nuclear.quants.rep2)
  ...
  ## Load gene quantifications for each cell fraction present in both cell lines
  total.quants.rep1 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[2], "/total/exp1/rep1/ENCFF205WUQ.tsv", sep =""))
  total.quants.rep2 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[2], "/total/exp1/rep2/ENCFF915JUZ.tsv", sep =""))
  cytosolic.quants.rep1 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[2], "/cytosolic/rep1/ENCFF309UUG.tsv", sep =""))
  cytosolic.quants.rep2 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[2], "/cytosolic/rep2/ENCFF730MKP.tsv", sep =""))
  insoluble_cytoplasmic.quants.rep1 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[2], "/insoluble_cytoplasmic/rep1/ENCFF358FIF.tsv", sep =""))
  insoluble_cytoplasmic.quants.rep2 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[2], "/insoluble_cytoplasmic/rep2/ENCFF282DWF.tsv", sep =""))
  membrane.quants.rep1 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[2], "/membrane/rep1/ENCFF272YYP.tsv", sep =""))
  membrane.quants.rep2 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[2], "/membrane/rep2/ENCFF086UXN.tsv", sep =""))
  nuclear.quants.rep1 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[2], "/nuclear/rep1/ENCFF651VFN.tsv", sep =""))
  nuclear.quants.rep2 <- read_tsv(paste(INPUT, RNA, CARGS$cell.line[2], "/nuclear/rep2/ENCFF534UDU.tsv", sep =""))
  cline.quants$HepG2 <- list(total.quants.rep1 = total.quants.rep1, total.quants.rep2 = total.quants.rep2,
                             cytosolic.quants.rep1 = cytosolic.quants.rep1, cytosolic.quants.rep2 = cytosolic.quants.rep2,
                             insoluble_cytoplasmic.quants.rep1 = insoluble_cytoplasmic.quants.rep1, insoluble_cytoplasmic.quants.rep2 = insoluble_cytoplasmic.quants.rep2,
                             membrane.quants.rep1 = membrane.quants.rep1, membrane.quants.rep2 = membrane.quants.rep2, 
                             nuclear.quants.rep1 = nuclear.quants.rep1, nuclear.quants.rep2 = nuclear.quants.rep2)
  ...
  quantifications <- 
    lapply(cline.quants, function(quants){
      ...
      ## Identify the cellular localization of non-coding transcripts
      quantifications$cellular_fraction <- "multiple"
      non.coding.transcripts <- quantifications$transcript_id[which(quantifications$non_coding==1)]
      non.coding.nuclear.transcripts <- 
        non.coding.transcripts[
          which((non.coding.transcripts %in% quants$nuclear.quants.rep1$transcript_id |
                   non.coding.transcripts %in% quants$nuclear.quants.rep2$transcript_id ) &
                  !(non.coding.transcripts %in% quants$cytosolic.quants.rep1$transcript_id & 
                      non.coding.transcripts %in% quants$cytosolic.quants.rep2$transcript_id &
                      non.coding.transcripts %in% quants$insoluble_cytoplasmic.quants.rep1$transcript_id &
                      non.coding.transcripts %in% quants$insoluble_cytoplasmic.quants.rep2$transcript_id &
                      non.coding.transcripts %in% quants$membrane.quants.rep1$transcript_id & 
                      non.coding.transcripts %in% quants$membrane.quants.rep2$transcript_id))]
      quantifications[match(non.coding.nuclear.transcripts, quantifications$transcript_id), "cellular_fraction"] <- "nuclear"
...
load.CAGE.tss <- function(ncores = CARGS$workers["avg"]){
  ...
  # Check if a precalculated version exists
  target.file <- paste(OUTPUT, CAGE,  "CTSS.RDS", sep = "")
  if(!(CARGS$new) & file.exists(target.file)){
  ...
  # # Save data set reformatted as GRanges
  # saveRDS(ctss, paste(OUTPUT, CAGE, "CAGEset_clustered_", cline, ".RDS", sep = ""))
  # # Save data set with individual ctss reformatted as GRanges
  # saveRDS(individual.ctss, paste(OUTPUT, CAGE, "CAGEset_individual_", cline, ".RDS", sep = ""))
}
###
...
retrieve.CHIPseq.experiments <- function(){
  ...
  targeted.chip.experiments <- 
    lapply(CARGS$cell.line, function(cline){
      ...
      metadata.orig <- read.table(paste(INPUT, CHIP, cline, "/bed/metadata.tsv", sep = ""), header = T, sep = "\t")
...
retrieve.eCLIPseq.experiments <- function(){
  ...
  targeted.clip.experiments <- 
    lapply(CARGS$cell.line, function(cline){
      # Load meta data file with file accessions
      metadata.orig <- read.table(paste(INPUT, eCLIP, cline, "/metadata_eCLIP_", cline, ".tsv", sep = ""), header = T, sep = "\t")
      metadata <- metadata.orig
      # Filter for hg19 assambly based files
      ...
      #ä Save table with download URLS
      file.access  <- metadata.orig$File.download.URL[match(encode.table$File.accession, metadata.orig$File.accession)]
      write.table(file.access, paste0(OUTPUT, "filtered.encode.eclipseq.download.urls_", cline, ".txt"), row.names = F, quote = F, col.names = F)
      
      # Return full paths of files to be included in further analyses
      target.files <- paste(INPUT, eCLIP, cline,"/", metadata$File.accession,".bed.gz", sep = "")
      target.files <- normalizePath(target.files)
      names(target.files) <- metadata$Experiment.target
      return(target.files)
    })
...
###
# This function parses the HGNC gene annoations and creates a mapping of genes 
# to biological functions and biological functions to genes sharing
# a biological function
###
retrieve.shared.protein.domains <- function(){
  
  # Check if a precalculated version exists
  target.file <- paste(OUTPUT, "protein.domains.RDS", sep="")
  if(!(CARGS$new) & file.exists(target.file)){
    protein.domains <- readRDS(target.file)
    return(protein.domains)
  }
  
  # Load gene domain group definitions from HGNC
  gene.groups <- read.table(paste(INPUT, GENEANNOT, "HGNC/", "hgnc_gene_groups.txt", sep = ""), 
                            header = T, sep = "\t", stringsAsFactors = F,quote = "\"",
                            fill = T)
  
  # Annotate genes which share a domain
  gene.groups <- gene.groups[!gene.groups$Gene.group.ID == "",]
  
  ## Resolve multi membership
  ...
  group.memberships <- group.memberships[names(group.memberships) != ""]
  ...
  # Aggregate results
  protein.domains <-  list(groups = groups, members = group.memberships)
  
  # # Aggregate results
  # domain.fu
  # apply(gene.groups, 1, function(domain.entry){
  #   
  # })
  
  # 
  # ## Functional annotation per group index
  # group.functions <- 
  # sapply(names(protein.domains$groups), function(group){
  #   gene.groups$Gene.group.name[match(group, gene.groups$Gene.group.ID)]
  # })
  
  
  # Save data set
  saveRDS(protein.domains, target.file)
...
###
# This function load knockdown gene expressin data sets for the K562 and HepG2 cell lines
###
load.shRNAseq.data <- function(){
  
  target.file <- paste(INPUT, SHRNA,  "knockdown.expression.profiles.RDS", sep = "")
  if(!(CARGS$new) & file.exists(target.file)){
    KOXP <- readRDS(target.file)
    return(KOXP)
  }
  
  knockdown.expression.profiles <- 
    lapply(CARGS$cell.line, function(cline){
      # Filter meta data file to select data
      metadata <- read_tsv(paste(INPUT, SHRNA, cline, "/metadata.tsv", sep =""))
      metadata <- metadata[metadata$`Output type` == "transcript quantifications", ]
      metadata <- metadata[metadata$Assembly == "hg19", ]
      metadata <- metadata[metadata$Assembly != "Non-specific target control-human", ]
      metadata <- metadata[metadata$Assembly != "/targets/Non-specific%20target%20control-human/", ]
      #metadata <- metadata[metadata$`Biological replicate(s)` == 1, ]
      metadata <- metadata[metadata$`Experiment target` != "Non-specific target control-human", ]
      metadata$`Experiment target` <- gsub("-human", "", metadata$`Experiment target`)
      factors <- metadata$`Experiment target`
      
      knockdown.expression.profiles <- 
        lapply(factors, function(factor){
          # Retrieve file accessions of replicates
          accession <- metadata$`File accession`[metadata$`Experiment target` == factor]
          # Load replicate knockdown data and remove transcripts that are not expressed
          knockdown.profile.rep1 <- read_tsv(file = paste(INPUT, SHRNA, "/", cline, "/", accession[1], ".tsv", sep = ""))
          knockdown.profile.rep1 <- knockdown.profile.rep1[knockdown.profile.rep1$FPKM > 0,]
          knockdown.profile.rep2 <- read_tsv(file = paste(INPUT, SHRNA, "/", cline, "/", accession[2], ".tsv", sep = ""))
          knockdown.profile.rep2 <- knockdown.profile.rep2[knockdown.profile.rep2$FPKM > 0,]
          # Identify transcripts that are expressed in both replicates
          common.transcripts <- intersect(knockdown.profile.rep1$transcript_id, knockdown.profile.rep2$transcript_id)
          common.knockdown.expression.profiles <- data.frame(rep1 = knockdown.profile.rep1$FPKM[match(common.transcripts, knockdown.profile.rep1$transcript_id)], 
                                                             rep2 = knockdown.profile.rep2$FPKM[match(common.transcripts, knockdown.profile.rep2$transcript_id)])
          # Calculate mean expression over both replicates
          common.valid.knockdown.profiles <- data.frame(transcript_id = common.transcripts, 
                                                        FPKM = apply(common.knockdown.expression.profiles,1, mean))
          common.valid.knockdown.profiles$log10_FPKM <- log10(common.valid.knockdown.profiles$FPKM)
          common.valid.knockdown.profiles$transcript_id <- strip.version(common.valid.knockdown.profiles$transcript_id)
          common.valid.knockdown.profiles
        })
      names(knockdown.expression.profiles) <- factors
      knockdown.expression.profiles
    })
  names(knockdown.expression.profiles) <- CARGS$cell.line
  
  saveRDS(knockdown.expression.profiles, target.file)
  return(knockdown.expression.profiles)
}
